(function ($) {
	...
	/*niceSelect*/
	let n_select_kecamatan = $('.niceselect_option_kecamatan').niceSelect();
	let n_select_kelurahan = $('.niceselect_option_kelurahan').niceSelect()
	$('#kecamatan').on('change', function () {
		let kelurahan = jQuery.parseJSON($('#data-kelurahan').val())
		let kecamatan = n_select_kecamatan.niceSelect('selected');
	...
	$('#kelurahan-checkout').on('change', function(){
		let kelurahan = jQuery.parseJSON($('#data-kelurahan').val())
		let kelurahan_dipilih = n_select_kelurahan.niceSelect('selected')
		let order_total_value = parseInt($('#order_total_value').val())
		// console.log(kelurahan)
		for (let i = 0; i < kelurahan.length; i++) {
			if(kelurahan[i]['kelurahan'] == kelurahan_dipilih){
				let total = order_total_value + parseInt(kelurahan[i]['harga'])
				$('#shipping').html(`${addCommas(kelurahan[i]['harga'])}`)
				$('#order_total').html(`${addCommas(total)}`)
				// console.log(total)
				// console.log($('#shipping').text())
				// console.log($('#order_total_checkout').text())
				break;
			}
		}
	})
